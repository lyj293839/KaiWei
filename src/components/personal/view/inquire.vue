  ...
  <div>
    ...
    <div>
        ...
        <ul class="inq_content" v-for="item in data.list">
            <li>
                <h1>我的询单</h1>
                <h2>产品：{{item.prod_name}}</h2>
                <h2>Cas：{{item.cas_no}}</h2>
                <h2>时间：{{item.create_time}}</h2>
                <h2>备注：{{item.remake}}</h2>
            </li>
            <li class="text1">
                <div class="text1_a">
                    <div>
                        <h2 v-if="!item.return_remake">暂未回复。（如长时间未回复，可主动与我们联系）</h2>
                        <h2 v-else>{{item.return_remake}}</h2>
                    </div>
                </div>
            </li>
        </ul>
...
export default {
  ...
  methods: {
    fetchat(){
      this.$post('/inquiryOrderList',this.queryList).then(res=>{
          if (res && res.code == 200){
              this.data = res.data
