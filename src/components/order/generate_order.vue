...
<template>
  <div style="background: #F9F9F9;">
    <div class="wind">
      ...
      <div>
        <div class="order_heard">订单号：{{orderData.order_sn}}</div>
        <div class="order_heard_a">您的订单已生成，等待您的付款。请牢记<span>订单号</span></div>
        ...
        <div class="order_content" v-else>
          <el-row type="flex" align="middle" justify="space-between">
            <el-col :span="14" class="plain">
              <el-button plain @click="play()" :class="san_show==1?'san_block':''">
              ...
              <el-button plain @click="san_show=4" :class="san_show==4?'san_block':''">
                <img src="@/assets/image/组 3626@2x.png" width="97" height="21">
                <div v-if='san_show==4'>
                  <div class="san"></div>
                  <i class="el-icon-check san_g"></i>
                </div>
            ...
            <el-col :span="5">
              <div>
                ...
                <span class="text2">￥{{orderData.total_money}}</span>
              </div>
            </el-col>
            <el-col :span="3">
              <el-button type="primary" style="width:138px;height:44px;" @click="se_order()">查看订单</el-button>
            </el-col>
          </el-row>


          <div v-show="san_show==1" class="payOrder">
            <div ref="loading" style="min-height:200px;min-width:200px;">
              <span id="qrcode" ref="qrcode"></span>
            </div>
            <div style="color:red;font-size:12px;line-height:30px;">
              （*请打开支付宝扫一扫,有效时间2分钟）
              <el-button style="margin:0px;padding:0px;width:auto;" size="mini" type="text" @click="loading()">刷新二维码</el-button>
            </div>
            
            <div>
              <el-button type="primary" style="width:138px;height:44px;margin-top:20px;" @click="play_wan()">已支付</el-button>
              <el-button style="width:138px;height:44px;margin-top:20px;" @click="play_wan()">取消支付</el-button>
            </div>
          </div>
          <div v-if="san_show==3">
          ...
          <div class="credit" v-else-if="san_show==4">
            <div style="text-align:left;">
              <span class="text1">您的信用额度为：</span>
              <span class="text2">￥{{newData.total_money}}</span>
            </div>
            <div style="text-align:center;margin:112px 0px 90px 0px;">
              <span class="text1">您的信用还剩余：</span>
              <span class="text2">￥{{(Number(newData.total_money)-Number(newData.used_money)).toFixed(2)}}</span>
              <span class="text3">本次消费额度为：</span>
              <span class="text2">￥{{orderData.total_money}}</span>
            </div>
            <div style="text-align:center;margin-bottom:56px;">
              <el-button type="primary" style="width:auto;" @click="use()" :disabled="!(Number(newData.total_money)-Number(newData.used_money))">确认使用</el-button>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    fetchat(){
      ...
      this.orderData = JSON.parse(localStorage.getItem('generate_order'))
    },
    loading(){
      this.play()
    },
    play(){
      this.san_show=1
      var order_sn = this.orderData.order_sn
      ...
      this.$post('/payOrder',{order_sn}).then(res=>{
          if (res && res.code == 200){
            this.$nextTick(() => {
              this.qrcode(res.data.alipay)
            })
          }else{
    ...
    use(){
      var obj = {order_sn:this.orderData.order_sn}
      this.$post('/creditPay',obj).then(res=>{
          if (res && res.code == 200){
